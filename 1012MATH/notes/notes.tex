...
\end{enumerate}
An $n\times n$ is invertible  \textbf{if and only if} its reduced row echelon form is $I_{n}$ THis occurs if and only if the rank is equal to $n$. A can only be invertible if the columns and rows are both linearly independent.}
