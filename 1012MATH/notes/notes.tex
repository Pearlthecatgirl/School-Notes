...
\par{There are 2 types of improper integrals, type 1 and type 2.

	The first type involves integrating over some infinite interval. An example being: \[
	\int_{\infty}^{a}{f\left(x\right)}\mathrm{dx}
	\] Where the upper boundary is infinite. This means that this integral is improper.

	The second type of improper integral is where the interval is finite, but the function itself is unbounded. An example being: \[
	\int_{-1}^{1}{\frac{1}{x}}\mathrm{dx}	
	\] There is a vertical asymptote which makes it improper.

	Sometimes the area from improper integrals are infinite but sometimes they are not. 
}
\subsection{Type One}
\par{Type one can either be from a to infinity or negative infinity to b. It is actually defined as: \[
\int_{a}^{\infty}{f\left(x\right)}\mathrm{dx}=\lim_{t}{infty}\int_{a}^{t}{f\left(x\right)}\mathrm{dx}
\] or \[
\int_{-\infty}^{b}{f\left(x\right)}\mathrm{dx}=\lim_{t}{-infty}\int_{t}^{b}{f\left(x\right)}\mathrm{dx}
\] 
This means that we can calculate these integrals by evaluating the integral normally up until the value $t$ and afterwards, take the limit as  $t$ tends to  $\infty$. If the limit exists, the improper integral is  \textbf{convergent}, otherwise it is known as \textbf{divergent}.

Type one improper integrals can also be doubly improper, being bounded from $-\infty\to\infty$. Then we can split the integral into one where the lower bound is negative infinity to some constant c and the other would be from c to positive infinity. Then the integral is convergent if and only if both improper integrals are convergent. The value of C is unimportant, simply choose the most convenient one.
}
\subsection{Type Two}
\par{Type 2 of integrals are of the following form: \[
\int_{a}^{b}{f\left(x\right)}\mathrm{dx}
\] Where $f$ is undefined at an end point (vertical asymptote). This gives 2 situations.

The first situation being if  $f\left(a\right)$ is undefined. Then we define: \[
\int_{a}^{b}{f\left(x\right)}\mathrm{dx}=\lim_{t\to a^{+}}\int_{t}^{b}{f\left(x\right)}\mathrm{dx}
\] 
Otherwise, if $f\left(b\right)$ is undefined, we define: \[
\int_{a}^{b}{f\left(x\right)}\mathrm{dx}=\lim_{t\to b^{-}}\int_{a}^{t}{f\left(x\right)}\mathrm{dx}
\] }
